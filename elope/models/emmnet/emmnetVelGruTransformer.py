...
class CrossModalTransformer(nn.Module):
    ...
    def forward(self, event_feat, imu_feat, range_feat):
        ...
        features = torch.stack([e_proj, i_proj, r_proj], dim=1)
        
        out = self.transformer(features)

        weights = F.softmax(self.modal_weights, dim=0) # (3,)
        weights = weights.unsqueeze(0).unsqueeze(-1) # (1, 3, 1)
        
        fused = torch.sum(out * weights, dim=1)
...
class MultiModalVelocityEstimatorTransformer(nn.Module):
    ...
    def create_model(cfg: str | Path | dict, device: str="cpu", **kwargs):
        ...
        model = MultiModalVelocityEstimatorTransformer(
    ...
    def forward(self, times, event_tensor, imu_tensor, range_tensor):
        ...
        # Extract features
        imu_feat   = self.imu_encoder(imu_tensor)           # (B, N_feat_imu)
        range_feat = self.range_encoder(range_tensor)       # (B, N_feat_range)
        event_feat = self.event_encoder(event_tensor)       # (B, N_feat_events)
        
        # Fusion
        fused_feat = self.fusion(event_feat, imu_feat, range_feat)

...
class RegularizedRegressor(nn.Module):
    ...
    """
    def __init__(self, input_dim: int, output_dim: int = 2, dropout: float = 0.3):
        """
...
class MultiModalVelocityEstimatorHorizontal(nn.Module):
    ...
    def __init__(
        ...
        vel_output_dim = range_output_dim
        self.vel_encoder = GRURangemeterEncoder(
            output_dim=vel_output_dim, 
            dropout=dropout
        )
    ...
    def create_model(cfg: str | Path | dict, device: str="cpu", **kwargs):
        ...
        model = MultiModalVelocityEstimatorHorizontal(
    ...
    def forward(self, times, event_tensor, imu_tensor, range_tensor, targets):
        ...
        # Targets is a tensor of size (B, T, 3)
        
        # Retrieve the vertical velocities
        vel_tensor = targets[:, :, -1:]    # (B, T, 1)
        ...
        vel_feat = self.vel_encoder(vel_tensor)
        
        # Retrieve fusion
        fused_feat, attention_weights = self.fusion(
            event_feat, imu_feat, range_feat, vel_feat
        )
    
        # Final prediction
        output = self.regressor(fused_feat)
        
        # Output is a tensor of shape (B, 3). 
        output = torch.cat([output, vel_tensor[..., 0]], dim=1)

...
class CfCRegressor(nn.Module):
    """
    A simple regressor based on CfC.
    
    Args:
        input_dim (int): Input dimensionality.
        output_dim (int): Output dimensionality. Defaults to 3.
        dropout (float): Dropout probability. Defaults to 0.3.
    """
    def __init__(self, input_dim: int, output_dim: int = 3, dropout: float = 0.3):
        super().__init__()

        wiring = AutoNCP(256, output_dim)

        self.CfCnet = CfC(input_dim, wiring, backbone_dropout=dropout)

    def forward(self, x, hx=None):
        x, hx = self.CfCnet(x, hx)
        return x, hx
...
class MultiModalVelocityEstimator(nn.Module):
    ...
    def __init__(self, 
        ...
        self.regressor = CfCRegressor(fusion_input_dim, output_dim, dropout)
    ...
    def forward(self, times, event_tensor, imu_tensor, range_tensor):
        ...
        if self.use_physics_aware:
            ...
            # Final prediction
            output, _ = self.regressor(fused_feat)

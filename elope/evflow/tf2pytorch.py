...
def load_model(path: str | Path, batch_norm: bool=True) -> nn.Module: 
    ...
    load_residual_layer(model.resnet_block[1], path, 1)

    # Load the decoder layers
    load_decoder_layer(model.decoder1, path, 0)
    load_decoder_layer(model.decoder2, path, 1)
    load_decoder_layer(model.decoder3, path, 2)
    load_decoder_layer(model.decoder4, path, 3)
